...
func (r *OtpRepo) FindByUserID(ctx context.Context, userID string) ([]model.Otp, error) {
	rows, err := r.pool.Query(ctx, `SELECT id, user_id, email, secret, expires_at, type, issuer, COALESCE(NULLIF(digits, '')::int, 0) AS digits, COALESCE(NULLIF(period, '')::int, 0) AS period, algorithm, valid FROM otp WHERE user_id = $1 ORDER BY id`, userID)
	if err != nil {
...
func (r *OtpRepo) FindByID(ctx context.Context, id int64) (*model.Otp, error) {
	rows, err := r.pool.Query(ctx, `SELECT id, user_id, email, secret, expires_at, type, issuer, COALESCE(NULLIF(digits, '')::int, 0) AS digits, COALESCE(NULLIF(period, '')::int, 0) AS period, algorithm, valid FROM otp WHERE id = $1`, id)
	if err != nil {
		return nil, fmt.Errorf("query: %w", err)
	}
	defer rows.Close()
...
func (r *OtpRepo) FindByUserIDAndEmail(ctx context.Context, userID, email string) (*model.Otp, error) {
	rows, err := r.pool.Query(ctx, `SELECT id, user_id, email, secret, expires_at, type, issuer, COALESCE(NULLIF(digits, '')::int, 0) AS digits, COALESCE(NULLIF(period, '')::int, 0) AS period, algorithm, valid FROM otp WHERE user_id = $1 AND email = $2`, userID, email)
	if err != nil {
		return nil, fmt.Errorf("query: %w", err)
	}
	defer rows.Close()
